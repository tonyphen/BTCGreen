...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Btcgreen server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Btcgreen server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Btcgreen features */
    { "btcgreen",               "masternode",             &masternode,             true,      true,       false },
    { "btcgreen",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "btcgreen",               "masternodebroadcast",    &masternodebroadcast,    true,      true,       false },
    { "btcgreen",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "btcgreen",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "btcgreen",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "btcgreen",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "btcgreen",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "btcgreen",               "darksend",               &darksend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use btcgreend, or the -server option to btcgreen-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> btcgreen-cli " + methodname + " " + args + "\n";
}
